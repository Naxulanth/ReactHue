  ...
  componentDidUpdate() {
    ...
    Object.keys(lights).forEach(lightId => {
      if (lightIds.includes(lightId))
        colorMap.push(objectToArray(getFormattedXYtoRGB(lights, lightId)));
    });
    // remove dupes
    ...
    if (colorMap.length > 1) {
      ...
      if (this.c) this.c.stop(this.id);
      this.main.current.style.borderColor =
        "rgb(" +
        getXYtoRGB(
          lights[lightIds[0]].state.xy[0],
          lights[lightIds[0]].state.xy[1],
          lights[lightIds[0]].state.bri
        ).join(",") +
        ")";
    }
  }
      ...
      insert.push(
        <Row key={uuidv4()}>
          <Col lg="12" sm="12" md="12" xl="12">
            <LightWidget lightId={lightId.toString()} />
          </Col>
        </Row>
      );
    });
